...
node_keys = { "addr:housenumber","aerialway","aeroway","amenity","barrier","highway","historic","leisure","natural","office","place","railway","shop","sport","tourism","waterway","traffic_sign","traffic_sign:forward","traffic_sign:forward" }
...
function node_function()
	local traffic_sign = Find("traffic_sign")

	if traffic_sign == "" then
		local traffic_sign_forward = Find("traffic_sign:forward")
		local traffic_sign_backward = Find("traffic_sign:backward")
		traffic_sign = traffic_sign_forward

		if traffic_sign == "" then
			traffic_sign = traffic_sign_backward
		else
			traffic_sign = traffic_sign_forward .. ";" .. traffic_sign_backward
		end
	...
	if highway ~= "" then
		if highway == "give_way" then
			traffic_sign = "DE:205;(give_way)"
	  elseif highway == "stop" then
			traffic_sign = "DE:206;(stop)"
		end
	end

	if traffic_sign ~= "" then
		Layer("traffic_sign", false)
		Attribute("traffic_sign", traffic_sign)

		local traffic_sign = string.gsub(traffic_sign, ",", ";")
		local index = string.find(traffic_sign, ";")

		local first_sign = traffic_sign

		if string.sub(first_sign, 1, #"DE:") == "de:" then
			first_sign = string.upper(first_sign)
		end

		if index ~= nil then
			first_sign = string.sub(traffic_sign, 1, index-1)
		end


		if (first_sign == "DE:274.1[30]") or (first_sign == "DE:274[30]") then
			first_sign = "DE:274.1"
		elseif (first_sign == "maxspeed") then
			first_sign = "DE:274"
		elseif (first_sign == "give_way") then
			first_sign = "DE:205"
		elseif (first_sign == "stop") then
			first_sign = "DE:206"
		elseif (first_sign == "maxheight") then
			first_sign = "DE:265"
		elseif (first_sign == "DE:274[10]") then
			first_sign = "DE:274-10"
		elseif (first_sign == "DE:274[30]") then
			first_sign = "DE:274-30"
		elseif (first_sign == "DE:274[50]") then
			first_sign = "DE:274-50"
		elseif (first_sign == "DE:274[70]") then
			first_sign = "DE:274-70"
		elseif (first_sign == "DE:241") then
			first_sign = "DE:241-30"
		elseif (first_sign == "DE:325") then
			first_sign = "DE:325.1"
		elseif (first_sign == "DE:262-3") then
			first_sign = "DE:262"
		elseif (first_sign == "DE:263[8]") then
			first_sign = "DE:263-8"
		elseif (first_sign == "DE:263[5]") then
			first_sign = "DE:263"
		elseif (first_sign == "DE:265[3]") then
			first_sign = "DE:265"
		elseif (first_sign == "DE:418") then
			first_sign = "DE:418-10"
		elseif (first_sign == "city_limit") then
			first_sign = "DE:310"
		elseif string.sub(first_sign, 1, #"DE:437") == "DE:437" then
			first_sign = "DE:437"
		elseif string.sub(first_sign, 1, #"DE:262") == "DE:262" then
			first_sign = "DE:262"
		elseif string.sub(first_sign, 1, #"DE:460-21") == "DE:460-21" then
			first_sign = "DE:460-21"
		end

		if traffic_sign == "destination" then
			Attribute("traffic_sign_image", "destination")
		-- elseif traffic_sign == "maxspeed" then
		-- 	Attribute("traffic_sign_image", "maxspeed")
		-- elseif traffic_sign == "city_limit" then
		-- 	Attribute("traffic_sign_image", "city_limit")
		else
			Attribute("traffic_sign_image", first_sign)
		end

		if Holds("direction") then
			local direction = Find("direction")
			if direction == "forward" then
				Attribute("in_way_direction", 1)
			elseif direction == "backward" then
				Attribute("in_way_direction", -1)
			else
				Attribute("direction", Find("direction"))
			end
		end
	end
	...
	if tourism == "information" and (information == "guidepost" or information == "route_marker") then
		local bicycle = Find("bicycle")
		Layer("signpost", false)
		Attribute("class", information)
		AttributeBoolean("bicycle", bicycle==yes)
	end
